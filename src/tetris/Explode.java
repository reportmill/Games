...
public static View explodePiece(View aView, int aRow, int aCol, int aStart)
{
    ...
    ParentView view = aView.getParent();
    ViewUtils.addChild(view, iview);
...
static void explodeFinished(View aView, List <View> thePieces)
{
    if(aView.getParent()!=null)
        ViewUtils.removeChild(aView.getParent(), aView);
        
    for(View v : thePieces) {
        ParentView par = v.getParent(); if(par==null) continue;
        ViewUtils.removeChild(par, v);
    }
    thePieces.clear();
}
